<template>
    <div class="pb-24">
        ...
        <div class="bg-gray-100 p-1 rounded-xl flex mb-6">
            <button v-for="v in ['month', 'week', '3day', 'day']" :key="v"
        ...
        <div class="flex justify-between items-center mb-4 px-2">
            <button @click="changeTime(-1)" class="p-2 text-primary hover:bg-white rounded-full"><i
        ...
        <div v-if="viewType === 'month'">
            <div class="bg-white p-4 rounded-3xl shadow-soft mb-6">
                <div class="grid grid-cols-7 gap-1 text-center mb-2">
                    <span v-for="d in ['日', '一', '二', '三', '四', '五', '六']" :key="d" class="text-xs text-subtext">{{ d
                    }}</span>
                </div>
                <div class="grid grid-cols-7 gap-1">
                    <div v-for="(day, i) in monthGrid" :key="i"
                        class="h-12 border-t border-gray-100 flex flex-col items-center pt-1"
                        :class="getDayTasks(day).length > 0 ? 'bg-primary/5' : ''" @click="day && (selectedDate = day)">
                        <span v-if="day"
                            :class="['text-xs', formatDate(day) === formatDate(new Date()) ? 'bg-primary text-white rounded-full w-5 h-5 flex items-center justify-center' : '']">{{
                                day.getDate() }}</span>
                        <div v-if="day" class="flex gap-0.5 mt-1">
                            <div v-for="n in Math.min(getDayTasks(day).length, 3)" :key="n"
                                class="w-1 h-1 bg-primary rounded-full"></div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="mt-6 space-y-3">
                ...
                <div v-if="getDayTasks(selectedDate).length === 0" class="text-center text-gray-400 text-sm">這一天是空白的畫布
                </div>
                <div v-for="task in getDayTasks(selectedDate)" :key="task.id"
                    ...
                    <div class="flex-1">
                        <span
                            :class="['font-bold text-sm', task.completed ? 'line-through text-gray-300' : 'text-text']">{{
                                task.title }}</span>
        ...
        <!-- Grid -->
        <div v-else class="flex flex-col bg-white rounded-3xl shadow-soft h-[600px] overflow-hidden">
            <div class="flex border-b border-gray-100 pl-12">
                <div v-for="date in displayDates" :key="date.toString()"
                    class="flex-1 text-center py-2 border-l border-gray-50">
                    <div class="text-xs text-subtext">{{ ['日', '一', '二', '三', '四', '五', '六'][date.getDay()] }}</div>
                    <div
                        :class="['font-bold', formatDate(date) === formatDate(new Date()) ? 'text-primary' : 'text-text']">
                        {{ date.getDate() }}</div>
                </div>
            </div>
            <div class="flex-1 overflow-y-auto relative">
                <div class="relative min-h-[1440px]">
                    <div v-for="h in 24" :key="h" class="flex h-[60px] border-b border-gray-50 box-border">
                    ...
                    <div class="absolute top-0 left-12 right-0 bottom-0 flex">
                        <div v-for="date in displayDates" :key="date.toString()"
                            ...
                            <div v-for="task in getDayTasks(date, true)" :key="task.id"
