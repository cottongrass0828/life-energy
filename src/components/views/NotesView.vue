<template>
    <div class="pb-24 space-y-6">
        ...
        <div class="bg-white p-5 rounded-3xl shadow-soft border-2 transition-colors"
            :class="editingId ? 'border-primary/50' : 'border-transparent'">
            <div v-if="editingId" class="text-xs text-primary font-bold mb-2">正在編輯筆記...</div>
            ...
            <div class="flex gap-2">
                <input type="text" v-model="noteForm.tags" placeholder="標籤 (用逗號分隔)"
                    class="flex-1 bg-gray-50 rounded-xl px-3 text-sm outline-none">
                <button v-if="editingId" @click="cancelEdit" class="text-gray-400 px-3 py-2 text-sm">取消</button>
                <button @click="handleSave"
                    class="bg-secondary text-teal-800 px-6 py-2 rounded-xl font-bold shadow-cute text-sm">{{ editingId ?
                        '更新' : '紀錄' }}</button>
            </div>
        </div>
        ...
        <div class="flex overflow-x-auto no-scrollbar gap-2 pb-2">
            <button @click="filterTag = ''"
                :class="['whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold', filterTag === '' ? 'bg-text text-white' : 'bg-white text-subtext']">全部</button>
            <button v-for="t in allTags" :key="t" @click="filterTag = t"
        ...
        <div class="space-y-4">
            <div v-for="note in filteredNotes.sort((a, b) => new Date(b.date) - new Date(a.date))" :key="note.id"
                ...
                <div class="flex-1 pb-2">
                    <div class="flex justify-between items-start mb-1">
                        ...
                        <div class="flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                            <button @click="startEdit(note)" class="text-gray-300 hover:text-primary text-xs"><i
                                    class="fas fa-pen"></i></button>
                            <button @click="$emit('delete-note', note.id)"
                    ...
                    <div class="flex flex-wrap gap-1">
                        <span v-for="t in note.tags" :key="t"
                            class="text-[10px] bg-gray-100 text-subtext px-2 py-0.5 rounded-md">#{{ t }}</span>
                    </div>
                </div>
            </div>
        </div>
